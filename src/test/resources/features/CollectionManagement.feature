Feature: Authenticated - Collection management
...
# Author: Kamala Kannan (TS-01/05/09)
  ...
  Rule: TS-09 Verify that an authenticated user can view a list of all their created private collections.
    ...
    Scenario Outline: TC-37 - Empty collections list returns 200
      When user sends authenticated GET to "<endpoint>" with no objects and valid key
      Then the response status code should be <statusCode>
      And the "Content-Type" header of response should be "application/json"
...
# Author Varshinee
  ...
  Rule: TS-10 Verify that an authenticated user can view the full details of a single object inside a specific collection.

    Scenario Outline: TC-40 Get an existing object
      When I send a GET request to collection "product" for object at row <rowNum>
    ...
    Scenario: TC-41 - Get an existing object from a collection with invalid API key
      Given the API key "invalid-key-xyz" is invalid but collection name and Object Id are valid
      When GET request is sent to the correct endpoint
      Then the response status code should be 403
      And the error message in response body should contain "Invalid API key"

    Scenario: TC-42 - Get an non existent object from a collection
      Given the API key and collection name are valid but Object Id "invalid-001" is invalid
      When GET request is sent to the correct endpoint
      Then the response status code should be 404
      And the error message in response body should contain "not found"

    Scenario: TC-43 - Get an object from a non existing collection
      Given the API key and Object Id are valid but collection name "invalid-collection" is invalid
      When GET request is sent to the correct endpoint
      Then the response status code should be 404
      And the error message in response body should contain "not found"
  ...
  Rule: TS-13 Verify that an authenticated user can perform a full update on an inventory record inside their collection.
    ...
    Scenario Outline: TC-051 & 54 PUT update object with valid data and response time is within acceptable limit
      ...
      Examples:
        ...
        | product        | ff8081819d82fab6019dd30612f25a92 |

    Scenario Outline: TC-052 PUT update with missing mandatory fields returns 400 Bad Request - DEFECT
